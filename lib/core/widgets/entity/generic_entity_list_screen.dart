...
class _GenericEntityListScreenState extends State<GenericEntityListScreen> {
  ...
  List<Map<String, dynamic>> get _filteredRecords {
    ...
    return _records.where((r) {
      return r.values.any((val) => val != null && val.toString().toLowerCase().contains(q));
    }).toList();
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: _isLoading ? null : _loadLiveRecords,
          ),
        ],
      ),
      body: Column(
        children: [
          ...
          Expanded(
            child: _buildBody(colors, displayed),
          ),
        ],
      ),
    );
  ...
  Widget _buildBody(AppPalette colors, List<Map<String, dynamic>> displayed) {
    ...
    if (displayed.isEmpty) {
      return Center(
        child: Text('No records found', style: TextStyle(color: colors.outline)),
      );
    }
